...
$sql2="SELECT distinct adscripcion
  FROM qsy_indicadores_vw 
  WHERE (ejercicio ='".$ej."' or '".$ej."' = '')
  and (declaracion ='".$decl."' or '".$decl."'='')
  and (tipo_decl ='".$tip."' or '".$tip."'='')
...
{  
  ...
  $tituloReporte = "Area de Adscripción";
...
$hoja->setCellValue("B12", "AREA DE ADSCRIPCIÓN");
...
$hoja->setCellValue('B'.$e, $rep2['adscripcion']); 

$documento->getActiveSheet()->getStyle('B'.$e.':F'.$e)
...
$x = 10;
$s = 10; //Numero de fila donde se va a comenzar a rellenar
 foreach ($reporv as $rep){$x++;}

$m = 13;
foreach ($reporte2 as $rep2){

$hoja->setCellValue('C'.$m,"=COUNTIFS('DETALLE REPORTE'!N".$s.":N".$x.",C12,'DETALLE REPORTE'!K".$s.":K".$x.",B".$m.')');
$hoja->setCellValue('D'.$m,"=COUNTIFS('DETALLE REPORTE'!N".$s.":N".$x.",D12,'DETALLE REPORTE'!K".$s.":K".$x.",B".$m.')');
$hoja->setCellValue('E'.$m,"=COUNTIFS('DETALLE REPORTE'!N".$s.":N".$x.",E12,'DETALLE REPORTE'!K".$s.":K".$x.",B".$m.')');
$hoja->setCellValue('F'.$m,"=COUNTIFS('DETALLE REPORTE'!N".$s.":N".$x.",F12,'DETALLE REPORTE'!K".$s.":K".$x.",B".$m.')');
$m++;
}

$hoja->setCellValue('C'.$m,"=COUNTIF('DETALLE REPORTE'!N".$s.":N".$x.',"PRESENTADA")');
$hoja->setCellValue('D'.$m,"=COUNTIF('DETALLE REPORTE'!N".$s.":N".$x.',"OMISA")');
$hoja->setCellValue('E'.$m,"=COUNTIF('DETALLE REPORTE'!N".$s.":N".$x.',"EN TIEMPO")');
$hoja->setCellValue('F'.$m,"=COUNTIF('DETALLE REPORTE'!N".$s.":N".$x.',"EXTEMPORANEA")');
$documento->getActiveSheet()->getStyle("C".$m.":F".$m)
      ->getFont()
      ->setSize(11)
      ->setBold(true);
...
$documento->getActiveSheet()->getColumnDimension('B')->setWidth(50);
...
$logo->setPath('imagenseccion/archivos/area'.$rfc.'.png');
$logo->setHeight(400);
// $logo->setWidth(400);
...
$writer->save('ReporteAreaAdscripción.xlsx');

header("Content-disposition: attachment; filename=ReporteAreaAdscripción.xlsx");
header("Content-type: MIME");
readfile("ReporteAreaAdscripción.xlsx");
}}}
?>