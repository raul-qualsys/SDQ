...
<script type="text/javascript">
	window.onload = function (){menuopc("sub3-8p","menu3-1n2","menu1-2n");}
</script>
<body>
	...
	<div id="main-content">
		...
		<div class="content" id="content">
			<div class="title0">
				<div class="title1">Países</div>
			</div>
			<div class="forms-container">
				<form method="POST" action="cat-paises.php">
					<div id="busqueda">
						...
						<div style="display: inline-block;text-align: right;">
							<button id="nuevo_elemento" class="boton-oculto" style="margin:1% 2%;height: auto;" type="button"><img src="<?php echo HTTP_PATH ?>/images/agregarpais.png" style="height: 40px; width: auto;" title="Agregar país"></button>
						</div>
					</div>
				</form>
			</div>
			<div class="forms-container">
				<div class="results">
						...
						if(isset($_POST['buscarcat']) || isset($_GET["p"])){
						    ...
						    $query = "SELECT pais,descr,estatus FROM qsy_paises WHERE (descr LIKE '%".$busqueda."%' or pais LIKE '%".$busqueda."%') ORDER BY descr";
						    $result=pg_query($conn,$query);
						    $total=pg_num_rows($result);
						    $elementos=10;
						    $paginas=ceil($total/$elementos);
						    $posicion=isset($_GET['p']) ? ($_GET['p']-1)*$elementos : 0;
						    $query = "SELECT pais,descr,estatus FROM qsy_paises WHERE (descr LIKE '%".$busqueda."%' or pais LIKE '%".$busqueda."%') ORDER BY descr LIMIT $elementos OFFSET $posicion";
							...
							<table cellspacing="0" cellpadding="0" class="search-table">
						        ...
						        <div style="text-align: center;">
							       ...
							       if(isset($_GET["p"]) && $_GET["p"]!=1){
							        	 ?>
							        	<a href="cat-paises.php?p=<?php echo $_GET['p']-1 ?>&b=<?php echo $busqueda ?>"><button class="seccion-anterior">Anterior</button></a>
							        	<?php 
							        }
						        	for($i=1;$i<=$paginas;$i++){
							        	...
							        	?>
							        	<a href="cat-paises.php?p=<?php echo $i ?>&b=<?php echo $busqueda ?>" style="text-decoration: none;">
							        ...
							        if((!isset($_GET["p"]) || $_GET["p"]!=$paginas) && $paginas!=1){
							        	 ?>
							        	<a href="cat-paises.php?p=<?php if(isset($_GET["p"])){echo $_GET['p']+1; }else echo 2;?>&b=<?php echo $busqueda ?>"><button class="seccion-siguiente">Siguiente</button></a>
							        	<?php 
							        }
						        	/* Fin de actualización */
							        ?>
		...
		<div id="nuevo-elemento">
			...
			<div style="text-align: center;">				
				...
				<input type="hidden" name="pais" id="validar" value="X">
